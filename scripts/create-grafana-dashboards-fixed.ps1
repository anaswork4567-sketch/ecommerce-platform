...
# First, add Prometheus data source
$dsPayload = @{
    name = "Prometheus"
    type = "prometheus"
    url = $PrometheusUrl
...
try {
    $response = Invoke-WebRequest -Uri "$GrafanaUrl/api/dashboards/db" `
        ...
        -Body $json1 `
        -UseBasicParsing -ErrorAction SilentlyContinue
    Write-Host "✓ Request Latency Dashboard created"
} catch {
...
try {
    $response = Invoke-WebRequest -Uri "$GrafanaUrl/api/dashboards/db" `
        ...
        -Body $json2 `
        -UseBasicParsing -ErrorAction SilentlyContinue
    Write-Host "✓ Payment Metrics Dashboard created"
} catch {
...
try {
    $response = Invoke-WebRequest -Uri "$GrafanaUrl/api/dashboards/db" `
        ...
        -Body $json3 `
        -UseBasicParsing -ErrorAction SilentlyContinue
    Write-Host "✓ System Health Dashboard created"
} catch {
...
Write-Host "✅ Grafana Dashboard Setup Complete!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host "`nGrafana URL: http://localhost:3000" -ForegroundColor Cyan
...
Write-Host "`nNavigate to Grafana to view your dashboards!" -ForegroundColor Green
